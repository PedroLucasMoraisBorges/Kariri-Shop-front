...
export function MyAddresses() {
  ...
  const [, setSelectedMethod] = useState<'delivery' | 'withdrawal'>(
    'withdrawal',
  )
  ...
  useEffect(() => {
    if (type === 'saleCart' && id) {
      fetchProductsCart(id)
    }

    if (type === 'saleProduct' && id) {
      fetchProductInfo(id)
    }
  }, [type, id])
          ...
          <div className="flex flex-col gap-6 px-3 lg:flex-row">
            <div className="flex w-2/3 flex-col gap-10">
              {addresses.map((address) => (
                <div
                  key={address.id}
                  className="relative flex items-start gap-3 rounded-lg bg-slate-100 p-4 shadow-md"
                >
                  <label
                  ...
                  >
                    <input
                      ...
                      checked={address.is_principal === true}
                      onChange={() => {
                    ...
                    <button
                      ...
                      className="absolute bottom-6 right-4 top-6 rounded-md bg-blue-500 px-3 py-1 text-sm font-semibold text-white hover:bg-blue-600"
                    >
                      Editar
                    </button>
                  </label>
